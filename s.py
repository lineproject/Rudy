        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                ...
                if msg.text == "Uni":
                 ...
                 if not fileTmp:
                     sendMessage(msg.to, "Image not found, maybe the account haven't post a single picture or the account is private")
                     print "[Command]Stalk,executed - no image found"
                 else:
                     image = upload_tempimage(client)
                     sendMessage(msg.to, format(image['link']))
    	            ...
    	            for song in data:
    		            client.sendMessage(msg.to, song[4])
                 ...
                 with requests.session() as s:
                     ...
                     for a in soup.select('.yt-lockup-title > a[title]'):
                         if '&List' not in a['href']:
                               client.sendMessage(msg.to,'http://www.youtube.com' + a['href'] + a['title'])
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                ...
                                sendMessage(msg.to,"error")
                elif "Chat " in msg.text:
                  bctxt = msg.text.replace("Chat ", "")
                  tt = client.getAllContactIds()
                  t = 3000
                  for i in tt:
                    while(t):
                      sendMessage(i, (bctxt))
